...
const server = http.createServer((req, res) => {
  ...
  if (req.method == 'GET') {
    refreshOpenhabExposedItems()
      .then(i => {
        sendResponse(res, 200, 'Exposed items updated');
      })
      .catch(e => {
        console.error(e);
        sendResponse(res, 500, 'Error to fetch exposed items');
      });
    return;
  }

  if (req.method != 'POST') {
    sendResponse(res, 404, 'Only GET/POST method available');
    return;
  }
  ...
  req.on('end', () => {
    ...
    try {
      ...
      if (TOKEN != token) {
        ...
        sendResponse(res, 403, 'Token is invalid');
        return;
      }
      ...
      isItemExposed(item)
        .then(exposed => {
          if (!exposed) {
            ...
            sendResponse(res, 400, `Item [${item}] not exposed`);
          } else {
            if (command == undefined) {
              console.warn(`Command : ${command} is not defined`);
              sendResponse(res, 400, `Missing command for item [${item}]`);
              return;
            }
            //POST to OpenHAB
